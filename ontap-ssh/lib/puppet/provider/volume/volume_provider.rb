...
Puppet::Type.type(:volume).provide(:volume_provider) do
	...
	def destroy
		Net::SSH.start( resource[:cmgmt] , resource[:cuser], :password => resource[:cpass] ) do |ssh|
			        output = ssh.exec!("volume unmount -vserver " + resource[:vserver_name] \
					+ " -volume " + resource[:volume] )
			        output = ssh.exec!("volume offline -vserver " + resource[:vserver_name] \
					+ " -volume " + resource[:volume] )
			        output = ssh.exec!("volume destroy -vserver " + resource[:vserver_name] \
					+ " -volume " + resource[:volume] \
					+ " -force" )
